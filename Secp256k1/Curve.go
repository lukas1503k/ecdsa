...
func (curve EllipticCurve) IsOnCurve(x, y *big.Int) bool {
	...
	ellipticCurveEquation := new(big.Int).Mul(x, x)
	ellipticCurveEquation.Mul(ellipticCurveEquation, x)

	ellipticCurveEquation.Add(ellipticCurveEquation, curve.B)
	ellipticCurveEquation.Mod(ellipticCurveEquation, curve.P)

	return ySquared.Cmp(ellipticCurveEquation) == 0
